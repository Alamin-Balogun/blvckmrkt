...
function ConversationList({conversations, loading, activeBuyerId, onSelect}) {
  if (loading) {
    return (
  ...
  if (conversations.length === 0) {
    return (
      <div style={{padding: "40px 20px", textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: 12}}>
        No conversations yet. Buyers can message you from your brand profile page.
      </div>
    );
  }
  return (
    <div style={{display: "flex", flexDirection: "column"}}>
      {conversations.map((c) => (
        <div
          key={c.buyer_id}
          onClick={() => onSelect(c.buyer_id)}
          style={{
            display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", cursor: "pointer",
            background: activeBuyerId === c.buyer_id ? "rgba(239,68,68,0.06)" : "transparent",
            borderBottom: "1px solid rgba(255,255,255,0.05)",
          }}>
          <div style={{
            width: 40, height: 40, borderRadius: "50%", flexShrink: 0, overflow: "hidden",
            background: "linear-gradient(135deg,#6366f1,#312e81)", display: "flex", alignItems: "center", justifyContent: "center",
          ...
          <div style={{flex: 1, minWidth: 0}}>
            ...
            <p style={{color: "rgba(255,255,255,0.35)", fontSize: 11, margin: "2px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap"}}>
              {c.last_sender === "brand" ? "You: " : ""}{c.last_message}
            </p>
          </div>
        </div>
      ))}
...
function Thread({buyerId, onBack}) {
  ...
  const load = () => {
    setLoading(true);
    getThread(buyerId).then(setData).catch(() => setData(null)).finally(() => setLoading(false));
  };

  useEffect(() => { load(); }, [buyerId]); // eslint-disable-line
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await sendMessage(buyerId, draft.trim());
      setDraft("");
      load();
    } catch (e) {
      console.error(e);
  ...
  return (
    <div style={{display: "flex", flexDirection: "column", height: "100%"}}>
      <div style={{display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.07)"}}>
        ...
        <span style={{color: "#fff", fontSize: 14, fontWeight: 800}}>{data?.buyer_name || "..."}</span>
      </div>
      ...
      <div style={{flex: 1, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 10}}>
        {loading ? (
          ...
          (data?.messages || []).map((m) => (
            <div key={m.id} style={{alignSelf: m.sender_type === "brand" ? "flex-end" : "flex-start", maxWidth: "75%"}}>
              <div style={{
                background: m.sender_type === "brand" ? "#ef4444" : "rgba(255,255,255,0.06)",
                color: m.sender_type === "brand" ? "#fff" : "rgba(255,255,255,0.8)",
                borderRadius: 12, padding: "9px 13px", fontSize: 12.5, lineHeight: 1.5,
              }}>
                {m.body}
              </div>
              <p style={{color: "rgba(255,255,255,0.2)", fontSize: 9, margin: "3px 4px 0", textAlign: m.sender_type === "brand" ? "right" : "left"}}>
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        {activeBuyerId ? (
          <Thread buyerId={activeBuyerId} onBack={() => setActiveBuyerId(null)} />
        ) : (
