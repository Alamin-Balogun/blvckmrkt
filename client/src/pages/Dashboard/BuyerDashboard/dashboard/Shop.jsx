...
const closePaymentModal = () => {
  try {
    const iframes = document.querySelectorAll('iframe[src*="flutterwave"], iframe[src*="ravemodal"]');
    iframes.forEach(iframe => iframe.remove());
    const flwElements = document.querySelectorAll('[class*="flutterwave-modal"], [class*="flw-modal"], [id*="flutterwave-modal"], [id*="flw-modal"]');
...
const back = () => {
  ...
  const [payment, setPayment] = useState({
    method: "flutterwave", cardNumber: "", expiry: "", cvv: "", cardName: "",
  });
                ...
                <div>
                  ...
                  <div style={{display:"flex",gap:10,marginBottom:18}}>
                    {PAYMENT_METHODS.map(m => {
                      ...
                      return (
                        <div key={m.id} onClick={() => { if (!disabled) setPayment({...payment,method:m.id}); }}
                          style={{flex:1,position:"relative",padding:"12px 8px",borderRadius:10,cursor:disabled?"not-allowed":"pointer",opacity:disabled?0.45:1,border:`1px solid ${payment.method===m.id?"#ef4444":"rgba(255,255,255,0.1)"}`,background:payment.method===m.id?"rgba(239,68,68,0.07)":"rgba(255,255,255,0.02)",display:"flex",flexDirection:"column",alignItems:"center",gap:5,transition:"all 0.2s"}}>
                          {disabled && (
                            <span style={{position:"absolute",top:-7,right:-4,background:"#eab308",color:"#000",fontSize:6,fontWeight:900,letterSpacing:"0.06em",textTransform:"uppercase",padding:"2px 5px",borderRadius:999}}>
                              Coming Soon
                            </span>
                          )}
...
export default function Shop() {
  ...
  const [wishToggling, setWishToggling] = useState(new Set());
  ...
  const handleWishToggle = async (p) => {
    ...
    if (wishToggling.has(pid)) return;
  ...
  const fetchProducts = useCallback(() => {
    ...
    getProducts(params)
      .then((d) => {
        ...
        const totalCount = Array.isArray(d) ? d.length : d?.total || d?.count || list.length;
        const totalPages = Array.isArray(d) ? 1 : d?.pages || d?.total_pages || 1;
        setProducts(list);
        setTotal(totalCount);
        setPages(totalPages);
        setLoading(false);
  ...
  const handleBuyNow = (p) => setPickerProduct({
  ...p,
  primary_image: p.primary_image || p.image_url || p.images?.[0]?.url || null,
  });
  ...
  return (
    <div>
      <style>{`
        ...
        @media(max-width:600px){.shop-grid{grid-template-columns:repeat(2,1fr)!important;gap:10px!important;}}
        @media(max-width:380px){.shop-grid{grid-template-columns:1fr!important;}}
      `}</style>
        ...
        <div style={{display: "flex", flexWrap: "wrap", gap: 6}}>
          <button
            ...
            style={{
              padding: "7px 15px",
              borderRadius: 99,
              border: `1px solid ${brandID === "" ? "rgba(239,68,68,0.5)" : "rgba(255,255,255,0.1)"}`,
              background: brandID === "" ? "rgba(239,68,68,0.1)" : "transparent",
              color: brandID === "" ? "#ef4444" : "rgba(255,255,255,0.45)",
              fontSize: 11,
              fontWeight: 700,
              cursor: "pointer",
              transition: "all 0.15s",
          ...
          {brands.map((b) => (
            <button
              ...
              style={{
                padding: "7px 15px",
                borderRadius: 99,
                border: `1px solid ${brandID === String(b.id) ? "rgba(239,68,68,0.5)" : "rgba(255,255,255,0.1)"}`,
                background: brandID === String(b.id) ? "rgba(239,68,68,0.1)" : "transparent",
                color: brandID === String(b.id) ? "#ef4444" : "rgba(255,255,255,0.45)",
                fontSize: 11,
                fontWeight: 700,
                cursor: "pointer",
                transition: "all 0.15s",
              ...
              return (
                <div
                  ...
                  <div style={{position: "relative"}}>
                    ...
                    {/* Save badge — sits below the tags badge on the left */}
                    {hasDiscount && (
                      <span
                        style={{
                          position: "absolute",
                    ...
                    <button
                      ...
                      disabled={wishToggling.has(String(p.id))}
                      style={{
                        position: "absolute",
                        top: 8,
                        right: 8,
                        ...
                        cursor: wishToggling.has(String(p.id)) ? "not-allowed" : "pointer",
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        transition: "all 0.18s",
                        opacity: wishToggling.has(String(p.id)) ? 0.6 : 1,
                        zIndex: 2,
                      }}
                      onMouseEnter={(e) => {
                      ...
                      <svg
                        ...
                        stroke={wishlistIds.has(String(p.id)) ? "#fff" : "rgba(255,255,255,0.8)"}
                        strokeWidth="2.5">
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                  ...
                  <div style={{padding: "12px 14px"}}>
                        ...
                        {p.sizes.slice(0, 4).map((sz) => (
                          <span
                            ...
                            style={{
                              ...
                              color:
                                sz.stock > 0 ? "rgba(255,255,255,0.4)" : "rgba(255,255,255,0.15)",
                              fontSize: 8,
                              padding: "1px 5px",
                              borderRadius: 3,
                              textDecoration: sz.stock === 0 ? "line-through" : "none",
                    ...
                    <div
                      ...
                      {/* Product card price: slashed original on top, red buyer price below */}
                      <div>
                        {hasDiscount && (
                          <p
                            style={{
              ...
              {Array.from({length: Math.min(5, pages)}, (_, i) => {
                ...
                return pg >= 1 && pg <= pages ? (
                  <button
                    ...
                    onClick={() => setPage(pg)}
                    style={{
                      background: pg === page ? "#ef4444" : "rgba(255,255,255,0.06)",
                      border: `1px solid ${pg === page ? "#ef4444" : "rgba(255,255,255,0.1)"}`,
