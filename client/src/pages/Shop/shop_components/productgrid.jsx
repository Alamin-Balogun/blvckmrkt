...
import {isOutOfStock} from "../../../utils/productMeta";
...
export default function ProductGrid() {
  ...
  const doAddToCart = async (productId, sizeId) => {
    ...
    // ctxAddToWishlist already no-ops for guests, so no extra guard needed here.
    if (quickAddWish) {
      if (!wishlist.includes(productId)) {
        await ctxAddToWishlist(productId);
      }
  ...
  const handleQuickAdd = async (p, e) => {
    e.preventDefault();
    e.stopPropagation();
    if (isOutOfStock(p)) return;
    // No login required for the cart side — ctxAddToWishlist below already
    // no-ops for guests, so the wishlist half just quietly skips for them.
  ...
  return (
    <>
      <div
        ...
        @media (max-width: 560px)  { .pg-cards { grid-template-columns: repeat(2, 1fr); gap: 10px; } }
        ${PRODUCT_CARD_CSS}
                  ...
                  products.map((p, i) => (
                    <motion.div
                      ...
                      <ProductCard
                        ...
                        cartLoading={loadingCartId === p.id}
                        wishLoading={loadingWishId === p.id}
